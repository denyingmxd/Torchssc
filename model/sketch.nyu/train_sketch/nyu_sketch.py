...
class NYUv2(BaseDataset):
    def __init__(self, setting, split_name, preprocess=None,
        ...
        self._seg_2d_sketch_root_folder = setting['seg_2d_sketch_root_folder']
    ...
    def __getitem__(self, index):
        ...
        seg_2d_sketch_path = os.path.join(self._seg_2d_sketch_root_folder,item_idx+'.npz')

        img, hha, tsdf, label_weight, depth_mapping_3d, gt, sketch_gt,seg_2d,seg_2d_sketch =\
            self._fetch_data(img_path, hha_path, label_weight_path, mapping_path, gt_path,seg_2d_path,seg_2d_sketch_path)
        ...
        if self._split_name == 'train':
            ...
            seg_2d_sketch = torch.from_numpy(np.ascontiguousarray(seg_2d_sketch)).float()
        ...
        output_dict = dict(data=img, label=gt, label_weight=label_weight, depth_mapping_3d=depth_mapping_3d,
                           ...
                           seg_2d=seg_2d,seg_2d_sketch=seg_2d_sketch)
        if self.preprocess is not None and extra_dict is not None:
            output_dict.update(**extra_dict)

        return output_dict

    def _fetch_data(self, img_path, hha_path, label_weight_path, mapping_path, gt_path,seg_2d_path, seg_2d_sketch_path,dtype=None):
        ...
        seg_2d =  np.load(seg_2d_path)['arr_0'].astype(np.float)
        seg_2d_sketch =  np.load(seg_2d_sketch_path)['arr_0'].astype(np.float).reshape(1, 60, 36, 60)
        return img, hha, tsdf, label_weight, depth_mapping_3d, gt, sketch_gt,seg_2d,seg_2d_sketch
