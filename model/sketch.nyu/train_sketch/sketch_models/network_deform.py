# encoding: utf-8
from .common import ProjectedSS_Conv
import torch
...
class STAGE1(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        self.ssc_DDA = nn.Sequential(ProjectedSS_Conv(feature, feature, [1, 6, 12, 18, 24, 30]),
                                     nn.Dropout3d(.1),
                                     nn.Conv3d(feature, 2, kernel_size=1, bias=True)
                                     )
        self.business_layer.append(self.ssc_DDA)
    ...
    def forward(self, tsdf, depth_mapping_3d, sketch_gt=None):
        ...
        pred_sketch = self.ssc_DDA(up_sketch2)

        return pred_sketch_raw, pred_sketch
...
main network2d
'''
class Network_deform(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_deform, self).__init__()
        self.business_layer = []
        self.dilate = 2
    ...
    def forward(self, img,depth_mapping_3d, tsdf, sketch_gt,seg_2d,seg_2d_sketch):
        ...
        pred_sketch_raw, pred_sketch = self.stage1(tsdf, depth_mapping_3d,sketch_gt)

        results={'pred_sketch_raw':pred_sketch_raw,'pred_sketch_refine':pred_sketch}
...
if __name__ == '__main__':
    model = Network_deform(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
