...
class STAGE2(nn.Module):
    ...
    def forward(self, raw, tsdf):
        ...
        pred_sketch_refine = self.classify_semantic[2](up_sem2)
...
class Network_s0_sketch_from_2d(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_s0_sketch_from_2d, self).__init__()
        self.business_layer = []
    ...
    def forward(self,img, depth_mapping_3d, tsdf, sketch_gt,seg_2d,seg_2d_sketch):
        ...
        results= self.stage2(seg_2d_sketch, tsdf)
        return results
