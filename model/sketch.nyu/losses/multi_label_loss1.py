...
class MultiLabelLoss1(nn.Module):
    def __init__(self, num_classes):
        super(MultiLabelLoss1, self).__init__()
    ...
    def forward(self, pred, target,label_weight):
        ...
        for i in range(b):
            ...
            target_b = target[i]
            pred_b = pred[i]
            target_weight_b = ((target_b.sum(dim=0)==1)&(target_b.max(dim=0)[0]<1.)).view(label_weight_b.shape[0])
    ...
    def get_loss(self, output, target):
        return -torch.sum(target * output) / len(output)