...
class STAGE3(nn.Module):
    def __init__(self,class_num, norm_layer, feature, bn_momentum):
        ...
        self.classify_semantic = nn.ModuleList([
            ...
            nn.Sequential(
                nn.Dropout3d(.1),
                nn.Conv3d(class_num, 12, kernel_size=1, bias=True)
            )]
        )

        self.fuse_probs= nn.Sequential(
            nn.Conv3d(class_num*2, class_num, kernel_size=1, bias=True),
            norm_layer(class_num, momentum=bn_momentum),
            nn.Sigmoid()
        )

        self.business_layer.append(self.fuse_probs)
        self.business_layer.append(self.classify_semantic)


    def forward(self, pred_feature_rgb, pred_feature_tsdf,pred_semantic_rgb,pred_semantic_tsdf):
        probs = torch.cat([pred_semantic_rgb,pred_semantic_tsdf], dim=1)
        prob_rgb = self.fuse_probs(probs)
        prob_tsdf = 1.-prob_rgb
        features = pred_semantic_rgb.mul(prob_rgb) + pred_semantic_tsdf.mul(prob_tsdf)
        pred_semantic = self.classify_semantic[0](features)
        return pred_semantic



class Network_s0_late_gated(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        super(Network_s0_late_gated, self).__init__()
        self.business_layer = []
    ...
    def forward(self,img, depth_mapping_3d, tsdf, sketch_gt,seg_2d):
        ...
        pred_semantic = self.stage3(pred_feature_rgb,pred_feature_tsdf,pred_semantic_rgb,pred_semantic_tsdf)
