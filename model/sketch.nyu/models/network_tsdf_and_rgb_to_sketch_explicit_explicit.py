...
class STAGE1(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        self.oper_rgb = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(feature//8, 1, kernel_size=3, padding=1, bias=False),
            norm_layer(1, momentum=bn_momentum),
            nn.ReLU(inplace=False),
        )
...
main network2d
'''
class Network_tsdf_and_rgb_to_sketch_explicit_explicit(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_tsdf_and_rgb_to_sketch_explicit_explicit, self).__init__()
        self.business_layer = []
...
if __name__ == '__main__':
    model = Network_tsdf_and_rgb_to_sketch_explicit_explicit(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
