...
class STAGE1(nn.Module):
    ...
    def forward(self, tsdf, depth_mapping_3d, sketch_gt=None):
        ...
        depth_mapping_3d = depth_mapping_3d.view(-1,1,60,36,60).float()
        depth_mapping_3d = self.oper1(depth_mapping_3d)
        completion1 = self.completion_layer1(depth_mapping_3d)
...
main network2d
'''
class Network_mapping(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_mapping, self).__init__()
        self.business_layer = []
        if eval:
    ...
    def forward(self, rgb, depth_mapping_3d, tsdf, sketch_gt=None):
        ...
        pred_sketch_raw = self.stage1(tsdf, depth_mapping_3d)
...
if __name__ == '__main__':
    model = Network_mapping(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
