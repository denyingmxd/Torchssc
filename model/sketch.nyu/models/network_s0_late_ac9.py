...
class channel_attention(nn.Module):
    ...
    def forward(self, x,weight):
        ratios = x.size(2)*x.size(3)*x.size(4)/(weight.sum(dim=[2,3,4], keepdim=True))
        weight = weight.expand(x.size(0),self.feature,x.size(2),x.size(3),x.size(4))
        x = x*weight
        x = self.pool(x)
        ratios=ratios.expand(x.size(0),self.feature,x.size(2),x.size(3),x.size(4))
        x = x.mul(ratios)
        x = self.conv(x)
        x = self.activation(x)
        return x
...
class STAGE3(nn.Module):
    def __init__(self,class_num, norm_layer, feature, bn_momentum):
        ...
        self.business_layer = []
        self.rgb_acm_0=channel_attention(feature)
        self.tsdf_acm_0=channel_attention(feature)
    ...
    def forward(self,rgbs,tsdfs,weights):
        rgb_fea = rgbs[0]
        tsdf_fea = tsdfs[0]
        weight = weights[0]
        rgb_fea_attn = self.rgb_acm_0(rgb_fea+tsdf_fea,weight)
        ...
        tsdf_fea = tsdfs[1]
        weight = weights[1]
        rgb_fea_attn = self.rgb_acm_1(rgb_fea + tsdf_fea, weight)
        tsdf_fea_attn = 1. - rgb_fea_attn
        ...
        tsdf_fea = tsdfs[2]
        weight = weights[2]
        rgb_fea_attn = self.rgb_acm_2(rgb_fea + tsdf_fea, weight)
        tsdf_fea_attn = 1.-rgb_fea_attn
...
class Network_s0_late_ac9(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        super(Network_s0_late_ac9, self).__init__()
        self.business_layer = []
    ...
    def forward(self,img, depth_mapping_3d, tsdf, sketch_gt,seg_2d):
        ...
        weights = self.get_voxel_weights(tsdf)
        pred_semantic = self.stage3(rgbs,tsdfs,weights)
        results={'pred_semantic':pred_semantic}
        return results

    def get_voxel_weights(self,depth_mapping_3d):

        maxpool = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 0))
        weight =((depth_mapping_3d<0)&(depth_mapping_3d>-1)).reshape(depth_mapping_3d.shape[0],1,60,36,60).float()
        weights = [weight]
        for i in range(2):
            weight = maxpool(weight)
