...
class Network_s0_tsdf_to_sketch(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_s0_tsdf_to_sketch, self).__init__()
        self.business_layer = []

        self.dilate = 2

        self.stage1 = STAGE1(class_num, norm_layer, resnet_out=resnet_out, feature=feature, ThreeDinit=True,
                 bn_momentum=bn_momentum, pretrained_model=pretrained_model, eval=eval, freeze_bn=False)

        self.business_layer.append(self.stage1)
    ...
    def forward(self,img, depth_mapping_3d, tsdf, sketch_gt,seg_2d):
        sketch = self.stage1(tsdf, depth_mapping_3d)
        pred_semantic= self.stage2(seg_2d, sketch)
        results={'pred_sketch_refine':sketch, 'pred_semantic':pred_semantic}
        return results
