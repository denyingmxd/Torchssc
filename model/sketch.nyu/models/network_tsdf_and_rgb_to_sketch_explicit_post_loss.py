...
class STAGE1(nn.Module):
    ...
    def forward(self, tsdf, depth_mapping_3d, feature_3d, sketch_gt=None):
        ...
        _, pred_sketch_now = torch.max(pred_sketch_raw, dim=1, keepdim=True)

        feature_3d = self.oper_rgb(feature_3d)

        pred_sketch_now = pred_sketch_now + feature_3d

        pred_sketch_now = self.oper2(pred_sketch_now)
...
class STAGE2(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        self.post_sketch = nn.Sequential(
            nn.Conv3d(12, 6, kernel_size=3, padding=1, bias=False),
            norm_layer(6, momentum=bn_momentum),
            nn.ReLU(),
            nn.Conv3d(6, 2, kernel_size=3, padding=1, bias=False),
        )

        self.business_layer.append(self.oper_sketch)
        self.business_layer.append(self.oper_sketch_rgb)
        self.business_layer.append(self.post_sketch)
    ...
    def forward(self, segres, depth_mapping_3d, pred_sketch_raw, pred_sketch_refine):
        ...
        seg_fea = segres + sketch_proi + sketch_refine_proi
        ...
        post_sketch = self.post_sketch(pred_semantic)
        return pred_semantic, post_sketch
...
class Network_tsdf_and_rgb_to_sketch_explicit_post_loss(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_tsdf_and_rgb_to_sketch_explicit_post_loss, self).__init__()
        self.business_layer = []
    ...
    def forward(self, rgb, depth_mapping_3d, tsdf, sketch_gt=None):
        ...
        pred_semantic, sketch_from_pred = self.stage2(feature3d, depth_mapping_3d, pred_sketch_raw, pred_sketch_refine)

        results = {'pred_semantic': pred_semantic, 'pred_sketch_raw': pred_sketch_raw,
                   'pred_sketch_refine': pred_sketch_refine,'sketch_from_pred':sketch_from_pred}
        return results
...
if __name__ == '__main__':
    model = Network_tsdf_and_rgb_to_sketch_explicit_post_loss(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
