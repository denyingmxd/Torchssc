...
class STAGE2(nn.Module):
    ...
    def forward(self, segres, depth_mapping_3d):
        '''
...
class Network_unet_predict_sketch(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_unet_predict_sketch, self).__init__()
        self.business_layer = []
    ...
    def forward(self, rgb, depth_mapping_3d, tsdf, sketch_gt=None):
        ...
        pred_semantic, _ = self.stage2(feature3d, depth_mapping_3d)
        sketch_from_pred =  self.post_sketch(pred_semantic)
        results={'pred_semantic':pred_semantic,'sketch_from_pred':sketch_from_pred}
        return results
...
if __name__ == '__main__':
    model = Network_unet_predict_sketch(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
