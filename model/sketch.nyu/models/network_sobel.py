...
main network2d
'''
class Network_sobel(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_sobel, self).__init__()
        self.business_layer = []
        self.sobel = Sobel3D()
    ...
    def forward(self, rgb, depth_mapping_3d, tsdf, sketch_gt=None):
        ...
        pred_sketch_raw_1 = self.sobel(tsdf).int()
...
if __name__ == '__main__':
    model = Network_sobel(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
