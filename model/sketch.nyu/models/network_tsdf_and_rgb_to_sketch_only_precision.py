...
class STAGE1(nn.Module):
    ...
    def forward(self, tsdf, depth_mapping_3d, feature_3d, sketch_gt=None, only_precison=False):
        '''
        extract 3D feature
        '''
        ...
        if only_precison:
            ...
            _, raw_pos = torch.max(pred_sketch_raw, dim=1, keepdim=True)
            _, refine_pos = torch.max(pred_sketch_refine, dim=1, keepdim=True)
            sketch_gt = sketch_gt.view(b, c, h, w, l)
            pred_sketch_raw[(raw_pos < 1) & (sketch_gt < 1)] = 0
            pred_sketch_refine[(refine_pos == 1) & (sketch_gt == 1)] = 1
...
class Network_tsdf_and_rgb_to_sketch_only_precision(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_tsdf_and_rgb_to_sketch_only_precision, self).__init__()
        self.business_layer = []
...
if __name__ == '__main__':
    model = Network_tsdf_and_rgb_to_sketch_only_precision(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
