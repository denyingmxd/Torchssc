...
class STAGE1(nn.Module):
    ...
    def forward(self,rgb,depth_mapping_3d):
        ...
        return semantic0,semantic1,semantic2
...
class STAGE2(nn.Module):
    ...
    def forward(self, tsdf,depth_mapping_3d):
...
#         self.conv = nn.Conv3d(num_channel, num_channel, kernel_size=1)
#         self.activation = nn.Sigmoid()  # todo modify the activation function
#     def forward(self, x,depth_mapping_3d):
#         weight = depth_mapping_3d<307200
#         weight = weight.float()
#         weight = weight.unsqueeze(1)
#         weight = weight.expand(weight.size(0), self.out_size, weight.size(2), weight.size(3))
#         x = x*weight
#         x = self.pool(x)
#         x = self.conv(x)
#         x = self.activation(x)
...
def channel_attention(num_channel,size):
    # todo add convolution here
    pool = nn.AdaptiveAvgPool3d((1,1,size[-1]))
    conv = nn.Conv3d(num_channel*2, num_channel, kernel_size=1)
...
class STAGE3(nn.Module):
    def __init__(self,class_num, norm_layer, feature, bn_momentum):
        ...
        self.business_layer = []
        self.rgb_acm_0=channel_attention(feature,(60,36,60))
        self.tsdf_acm_0=channel_attention(feature,(60,36,60))
        self.rgb_acm_1=channel_attention(feature,(30,18,30))
        self.tsdf_acm_1=channel_attention(feature,(30,18,30))
        self.rgb_acm_2=channel_attention(feature*2,(15,9,15))
        self.tsdf_acm_2=channel_attention(feature*2,(15,9,15))
        ...
        self.business_layer.append(self.rgb_acm_0)
        self.business_layer.append(self.tsdf_acm_0)
    ...
    def forward(self,rgbs,tsdfs ):
        rgb_fea = rgbs[0]
        tsdf_fea = tsdfs[0]
        rgb_fea_attn = self.rgb_acm_0(torch.cat([rgb_fea,tsdf_fea], dim=1))
        rgb_fea_attn = rgb_fea_attn.expand(rgb_fea_attn.size(0), rgb_fea_attn.size(1), rgb_fea.size(2), rgb_fea.size(3), rgb_fea.size(4))
        tsdf_fea_attn = 1. - rgb_fea_attn
        fuse_fea_0 = rgb_fea.mul(rgb_fea_attn) + tsdf_fea.mul(tsdf_fea_attn)

        rgb_fea = rgbs[1]
        tsdf_fea = tsdfs[1]
        rgb_fea_attn = self.rgb_acm_1(torch.cat([rgb_fea, tsdf_fea], dim=1))
        rgb_fea_attn = rgb_fea_attn.expand(rgb_fea_attn.size(0), rgb_fea_attn.size(1), rgb_fea.size(2), rgb_fea.size(3), rgb_fea.size(4))
        tsdf_fea_attn = 1.-rgb_fea_attn
        ...
        rgb_fea = rgbs[2]
        tsdf_fea = tsdfs[2]
        rgb_fea_attn = self.rgb_acm_2(torch.cat([rgb_fea,tsdf_fea], dim=1))
        rgb_fea_attn = rgb_fea_attn.expand(rgb_fea_attn.size(0), rgb_fea_attn.size(1), rgb_fea.size(2), rgb_fea.size(3), rgb_fea.size(4))
        tsdf_fea_attn = 1.-rgb_fea_attn
        ...
        pred_semantic = self.classify_semantic[1](pred_semantic)+self.conv0(fuse_fea_0)
...
class Network_s0_late_ac7(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
        ...
        super(Network_s0_late_ac7, self).__init__()
        self.business_layer = []
    ...
    def forward(self,img, depth_mapping_3d, tsdf, sketch_gt,seg_2d):
        rgb_semantic0,rgb_semantic1,rgb_semantic2 = self.stage1(seg_2d,depth_mapping_3d)
        tsdf_semantic0,tsdf_semantic1, tsdf_semantic2 = self.stage2(tsdf,depth_mapping_3d)
        rgbs=[rgb_semantic0,rgb_semantic1,rgb_semantic2]
        tsdfs=[tsdf_semantic0,tsdf_semantic1,tsdf_semantic2]
