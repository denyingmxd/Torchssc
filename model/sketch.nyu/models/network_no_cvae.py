...
class STAGE2(nn.Module):
    ...
    def forward(self, feature2d, depth_mapping_3d, pred_sketch_raw):
...
main network2d
'''
class Network_no_cvae(nn.Module):
    def __init__(self, class_num, norm_layer, resnet_out=2048, feature=512, ThreeDinit=True,
                 bn_momentum=0.1, pretrained_model=None, eval=False, freeze_bn=False):
        super(Network_no_cvae, self).__init__()
        self.business_layer = []
    ...
    def forward(self, rgb, depth_mapping_3d, tsdf, sketch_gt=None):
        ...
        if config.use_sketch_gt:
            if not (sketch_gt is None):
                b, c, h, w, l = tsdf.shape
                # sketch_gt.float()
                if config.drop_rate<1:
                    sketch_gt = sketch_gt.reshape(-1,)
                    full_place = (sketch_gt>0).nonzero()
                    drop_ind = torch.from_numpy(np.random.choice([i for i in range(len(full_place))],
                                                                 int(len(full_place)*config.drop_rate), replace=False)).long().cuda()
                    # probs = (torch.ones(len(full_place),dtype=torch.float)/len(full_place)).cuda()
                    # drop_ind = probs.multinomial(int(len(full_place)*config.drop_rate),replacement=False)

                    sketch_gt[full_place[drop_ind]] = 0



                pred_sketch_raw_1 = sketch_gt.view(b, 1, h, w, l)
                ...
                # pred_sketch_raw_1[pred_sketch_raw_1 == 0] = -2
                # pred_sketch_raw_0[pred_sketch_raw_0 == 1] = -2
            ...
            if sketch_gt is None:
                print('sketch gt not passed but want to use')
                exit()
        else:
            pred_sketch_raw = self.stage1(tsdf, depth_mapping_3d, sketch_gt)
        ...
        if self.training:
            ...
            return pred_semantic, _, pred_sketch_raw
...
if __name__ == '__main__':
    model = Network_no_cvae(class_num=12, norm_layer=nn.BatchNorm3d, feature=128, eval=True)
    # print(model)
