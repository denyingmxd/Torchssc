...
class Evaluator(object):
    def __init__(self, dataset, class_num, image_mean, image_std, network,
                 ...
                 verbose=False, save_path=None, show_image=False,modelname=None):
        # print(network2d)
        self.eval_time = 0
        ...
        self.csv_out=save_path+'/{}.csv'.format(modelname)
        ...
        if self.csv_out is not None:
            if config.model_name in ['network2d','network_tsdf_and_rgb_to_sketch']:
                with open(self.csv_out, 'w') as f:
                    fields = ['trained on','epoch','IoU','precision','recall','empty','ceil','floor','wall',
                              'win','chair','bed','sofa','table','tvs','furn','objs','avgs',
                              'sketch_IoU','sketch_precision','sketch_recall', 'raw_sketch_IoU','raw_sketch_precision','raw_sketch_recall',
                              'empty', 'ceil', 'floor', 'wall',
                              'win', 'chair', 'bed', 'sofa', 'table', 'tvs', 'furn', 'objs', 'avgs',
                              'empty', 'ceil', 'floor', 'wall',
                              'win', 'chair', 'bed', 'sofa', 'table', 'tvs', 'furn', 'objs', 'avgs'
                              ]
                ...
                with open(self.csv_out, 'w') as f:
                    fields = ['trained on', 'epoch', 'IoU', 'precision', 'recall', 'empty', 'ceil', 'floor', 'wall',
                              'win', 'chair', 'bed', 'sofa', 'table', 'tvs', 'furn', 'objs', 'avgs',
                              'sketch_IoU', 'sketch_precision', 'sketch_recall', 'raw_sketch_IoU',
                              'raw_sketch_precision', 'raw_sketch_recall',
                              'empty', 'ceil', 'floor', 'wall',
                              'win', 'chair', 'bed', 'sofa', 'table', 'tvs', 'furn', 'objs', 'avgs',
                              'empty', 'ceil', 'floor', 'wall',
                              'win', 'chair', 'bed', 'sofa', 'table', 'tvs', 'furn', 'objs', 'avgs'
    ...
    def run(self, model_path, model_indice, log_file=None, log_file_link=None):
        ...
        for i,model in enumerate(models):
            ...
            if len(self.devices ) == 1:
                result_line = self.single_process_evalutation(i)
            else:
                result_line = self.multi_process_evaluation(i)
    ...
    def single_process_evalutation(self,i):
    ...
    def multi_process_evaluation(self,i):
        start_eval_time = time.perf_counter()
        ...
        for d in range(nr_devices):
            ...
            p = self.context.Process(target=self.worker,
                                     args=(shred_list, device,self.models[i]))
            procs.append(p)
        ...
        result_line = self.compute_metric(all_results,i)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line

    def worker(self, shred_list, device,model=None):
        ...
        for idx in shred_list:
            dd = self.dataset[idx]
            results_dict = self.func_per_iteration(dd, device,model)
            self.results_queue.put(results_dict)

    def func_per_iteration(self, data, device,model=None):
